...
Ideally, each unit test should be narrowly focused on testing that a specific function called in a specific way has a predictable outcome. For instance, an exception is raised or some specific value is returned. By limiting the scope of each individual test, the running time is reduced such each test completes in less than ½ a second. Furthermore, the limited scope helps to narrow down potential future issues if and when some unit tests fail. Writing automated unit tests for machine learning models in this environment can be cumbersome for a number of reasons:
